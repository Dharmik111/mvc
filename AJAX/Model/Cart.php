...
class Cart extends \Model\Core\Table
{
    ...
    public function addItemToCart($product, $quantity = 1, $addMode = false)
    {
        ...
        $cart_item = \Mage::getModel('Model\Cart\Item')->fetchRow($query);
        if ($cart_item) {
            $cart_item->quantity += $quantity;
            $cart_item->price = $cart_item->getRowTotal();
            $cart_item->save();
            return true;
        }

        $cart_item = \Mage::getModel('Model\Cart\Item');
        $cart_item->cartId = $this->cartId;
        $cart_item->productId = $product->productId;
        $cart_item->basePrice = $product->price;
        $cart_item->quantity = $quantity;
        $cart_item->price = $cart_item->getRowTotal();
    ...
    {
        return ($this->getTotal($items) - $this->getTotalDiscount($items) + $this->shippingAmount);
    }
